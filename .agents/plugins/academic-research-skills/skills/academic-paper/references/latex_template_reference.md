...
```bash
pandoc paper.md -o paper.pdf --pdf-engine=xelatex --bibliography=references.bib --csl=apa.csl -V geometry:margin=1in -V fontsize=12pt -V linestretch=2
```

### Markdown → PDF (with Chinese)
```bash
pandoc paper.md -o paper.pdf --pdf-engine=xelatex -V CJKmainfont="Noto Sans CJK TC" --bibliography=references.bib --csl=apa.csl
```

...
```bash
python scripts/claim_strength_drift_disposition.py build --finding-set <claim-strength-drift-findings.json> --author-input <explicit-author-dispositions.json> --final-draft <exact-revised-draft> --revision-evidence-bundle <exact-revision-evidence-bundle.json> --output <claim-strength-drift-disposition.json>
```

```bash
python scripts/claim_strength_drift_disposition.py validate --finding-set <claim-strength-drift-findings.json> --sidecar <claim-strength-drift-disposition.json> --final-draft <exact-revised-draft> --revision-evidence-bundle <exact-revision-evidence-bundle.json> --event-artifact 'ASSERTED-EVENT-e6-1=/absolute/run-local/event-1.raw' [--event-artifact 'ASSERTED-EVENT-e6-N=/absolute/run-local/event-N.raw' ...]
```
